...
public class LessonSeedData {
    ...
    public static List<Lesson> getLessons() {
        ...
        introduction.setTasks(List.of(
                new Task(introduction.getLessonId(), "Initialize a Git repository using the keyword init",
                        "Run the command to initialise Git", "git init",
                        "Use git inside your project folder", 1, DifficultyLevels.EASY),
                new Task(introduction.getLessonId(), "Check the current Git version", "Check your installed Git version using --version",
                        "git --version", "Try running --version in your terminal, and the Git version should show",
                        2, DifficultyLevels.EASY),
                new Task(introduction.getLessonId(), "View repository status", "Check the status of the repository. See what has changed so far",
        ...
        filesAndCommits.setTasks(List.of(
                new Task(filesAndCommits.getLessonId(), "Stage a file", "Let's add a file to the staging area, for this case, example.txt will do.",
                        "git add example.txt",
                ...
                new Task(filesAndCommits.getLessonId(), "Commit changes", "Create a commit with a message. Let's say 'Added first file'",
                ...
                new Task(filesAndCommits.getLessonId(), "Stage all changes", "Stage all modified files by adding them all to the staging index. The '.' includes all modified files.",
                        "git add .", "Be careful when using add .", 3, DifficultyLevels.EASY),
                new Task(filesAndCommits.getLessonId(), "View commit history", "See a log of previous commits made within the repository",
        ...
        branchingBasics.setTasks(List.of(
                ...
                new Task(branchingBasics.getLessonId(), "Switch to a branch", "You will need the Git command to check out the branch you just created, called my-new-branch",
                        "git checkout my-new-branch",
                        "The branch is called my-new-branch, don't forget the key word checkout", 2, DifficultyLevels.MEDIUM),
                ...
                new Task(branchingBasics.getLessonId(), "List all branches", "View all branches in your repository",
                        "git branch -a", "Use the branch command with the flag that shows ALL branches and highlights the current one.",
        ...
        mergingAndConflicts.setTasks(List.of(
                new Task(mergingAndConflicts.getLessonId(), "Merge a branch",
                        "Let's just say we did not delete the previous branch, and that we are currently in a different branch, so let's combine changes from one branch into another. Let's merge into branch 'my-new-branch'",
                ...
                new Task(mergingAndConflicts.getLessonId(), "View merge conflicts", "After merging, check the status of the repository to check if any files have conflicts that need resolving.",
                        "git status",
                        "Files with conflicts will be marked as 'both modified'; you must check the status of these files before committing. Use this command to find files that need checking before completing the merge.", 2, DifficultyLevels.MEDIUM),
                new Task(mergingAndConflicts.getLessonId(), "Resolve a conflict", "Edit the conflicted files and mark as resolved. Let's say we resolved conflicts locally, let's add my-new-file to the staging index.",
                ...
                new Task(mergingAndConflicts.getLessonId(), "Finalize the merge", "Complete the merge after resolving conflicts. To complete merges, a final commit is required. Git provides a commit message for you after conflicts are resolved.",
                        "git commit",
        ...
        remoteRepositories.setTasks(List.of(
                new Task(remoteRepositories.getLessonId(), "Clone a repository",
                        ...
                        "Clone the repository using its public URL. The URL is: https://github.com/onaly/storage", 1, DifficultyLevels.MEDIUM),
                new Task(remoteRepositories.getLessonId(), "Push a local branch to a remote repository",
                        "Push your local branch to the remote repository. The branch is my-new-branch, and the remote is origin", "git push origin my-new-branch",
        ...
        rebaseAndSquashing.setTasks(List.of(
                ...
                new Task(rebaseAndSquashing.getLessonId(), "Force Push After Rebase",
                        "Force push your rebased branch to the remote after rewriting history",
                        "git push --force",
